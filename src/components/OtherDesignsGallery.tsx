...
import { OTHER_DESIGNS_ITEMS } from '../data/galleryData';
import type { GalleryItem } from '../data/galleryData';


interface OtherDesignsGalleryProps {
  onNavigate: (path: string) => void;
  onOpenLightbox: (items: GalleryItem[], index: number) => void;
}

export const OtherDesignsGallery: React.FC<OtherDesignsGalleryProps> = ({
  onNavigate,
  onOpenLightbox,
}) => {
  ...
  const filteredItems =
    activeFilter === 'all'
      ? OTHER_DESIGNS_ITEMS
      : OTHER_DESIGNS_ITEMS.filter((item) => item.category === activeFilter);
  ...
  return (
    <div className="flex flex-col w-full pt-16 pb-28 bg-[#faf9f5]">
      {/* Header Info */}
      <div className="px-4 md:px-8 pt-4 pb-3 flex flex-col gap-1 max-w-7xl mx-auto w-full">
        <div className="flex items-center justify-between font-inter text-xs uppercase tracking-widest text-[#6A6A6A]">
          <span>Archive 2023–2025</span>
          <span className="text-[#516600] font-semibold">14 Artifacts Selected</span>
        </div>
        <h1 className="font-space text-3xl md:text-5xl uppercase tracking-tight text-[#000000] font-bold leading-none mt-1">
          04 — Other Designs
        </h1>
        <p className="font-geist text-xs md:text-sm text-[#444748] max-w-xl mt-1 leading-relaxed">
          Personal, experimental, typography & digital artwork. Uncommissioned explorations in Swiss grids, brutalist glyphs, chrome renderings, and printed matter.
        </p>
      </div>
      ...
      {/* Visual Image Feed */}
      <div className="px-4 md:px-8 pt-6 pb-8 max-w-7xl mx-auto w-full">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredItems.map((item, idx) => (
            <div
              key={item.id}
              onClick={() => onOpenLightbox(filteredItems, idx)}
              className="group relative w-full overflow-hidden rounded bg-[#eeeeea] cursor-pointer hover:shadow-md transition-all"
            >
              <img
                src={item.src}
                alt={item.alt}
      ...
      <section className="px-4 md:px-8 mt-4 max-w-7xl mx-auto w-full">
        ...
        >
          <div className="flex items-center justify-between font-inter text-xs uppercase tracking-widest text-[#6A6A6A]">
            <span>Curated Archive 04 / 04</span>
          ...
          <p className="font-geist text-xs md:text-sm text-[#6A6A6A] mt-2 leading-relaxed">
            Personal bio, graphic design capabilities, design process, and direct project contact.
          </p>
          <div className="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded bg-[#000000] text-[#ffffff] font-inter text-xs uppercase tracking-wider group-hover:bg-[#516600] transition-colors">
            <span>Proceed to 05 About Me</span>
